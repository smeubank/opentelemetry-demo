...
//     so a Sentry issue, the Jaeger trace and Supabase edge_logs share one trace_id;
...
// (<trace>-<span>-<sampled>) so captured errors carry the OTel trace_id.
  ...
  const run = async (): Promise<Response> => {
    ...
    try {
      ...
      if (SENTRY_DSN) {
        ...
        // Edge isolates can freeze after the response; flush so the event is sent.
